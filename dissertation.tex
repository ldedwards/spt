...
here has an extensive literature can be found in \cite{ShreveKaratzas1998}.
...
We briefly introduce Itô calculus, beginning with definition of a
simple process and the the Itô integral for simple functions, and
then move to the more general definition of the Itô integrals. Square-integrable
(random) adapted processes, introducing the notion of measurability. 
\begin{defn}
...
fullfils this requirement in SPT.
...
We have now defined the core features of SPT, namely the constintuent stocks and
...
and expressions (\ref{eq:marketweights}) and (\ref{eq:relativemarketweights})
...
The diversity condition asserts that no single company\textquoteright s
...
is defined in terms of the market portfolio $\mu(\centerdot)$ (\ref{eq:defnmarketportfolio})
by
\begin{equation}
...
Where the master equation (\ref{thm:MasterEquation}) for the DWP
is
\begin{equation}
