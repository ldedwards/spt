...
SPT models prices of assets using continuous semimartingales. By convention the
...
The theory is often presented referring to nondividend paying stocks for
...
for the formulaic construction of wellperforming portfolios that
...
proved that under an additional \textit{nondegeneracy} condition on the stock
...
exist in volatilitystabilised markets (\cite{banner2008short} generalised
volatilitystabilised markets (\cite{pickova2014generalized}), and Markovian
intrinsically volatile models (Proposition 2 of \cite{fernholz2009}).

\item \textbf{Rankbased:} In Rank-based methods, the drift and volatility
...
arbitrage has not been addressed yet. A very simple case of a rankbased model,
...
%6, demonstrating that our adjustments of the diversityweighted portfolio would
...
\subsection{LogReturns}

We define the daily logreturn of a stock price as the daily increment
of the natural logarithm of this price. If $X(t)$ represents the stock
price at time $t$, then the logreturn $R(t)$ is defined as
...
\begin{defn} [Simple Process]
  ...
  $k=0,1,...,n1$ such that
  \begin{equation}
    f(t,\omega)=\xi_{0}(\omega)\mathbf{1}_{\{0\}}(t)+\sum_{k=0}^{n1}\xi_{k}(\omega)\mathbf{1}_{(t_{k},t_{k+1}]}(t),
  \end{equation}
...
semimartingales, which is fairly standard in continuoustime financial theory
...
interestrate process for the money-market, $B(0)=1$. The price process
...
more natural when considering longterm behaviour (see e.g.
\cite{fernholz1982}).
...
\begin{prop} [
  ...
  (nonnegative definite matrix-valued) \textit{covariance process} $a_{ij}(t)$
  are defined as
...
\begin{prop} [{\cite[Equation 2.2]{fernholz2009}} Wealth Process of a Portfolio]
  ...
  where we define the process $b(\cdot)$, called the \textit{rateof-return}, and the
...
portfolios with nonnegative weights, the excess growth rate is non-negative,
...
where we define $\tau_{ij}^{\pi}$, the matrixvalued \textit{covariance process}
...
\begin{lem} [
  ...
  and in particular, for a longonly portfolio $\pi(\cdot)$ we have that
...
In this section we shall show that certain realvalued functions of the market
...
of the market weights, as these are used to generate diversityweighted
...
\newpage
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{DiversityWeighted Portfolio (DWP)}
...
timeinterval $[0,T]$, and if the strong non-degeneracy condition
...
the diversityweighted portfolio (DWP) that can be show to provide such relative
arbitrages over the market portfolio.

The diversityweighted portfolio $\pi^{(p)}(\cdot)$, is a functionally generated
...
for the wealth $V^{\pi^{(p)}}(\cdot)$ of the diversityweighted portfolio
$\pi^{(p)}$ of (\ref{eq:DWP}).
...
covers the 4year period 2014-2017 for European equities that compromise the
...
of the Eurostoxx 600 index over the period 20142017, as measured by
...
diversity is meanreverting, however with only 4 years of data, it is hard to
...
accompany bubbles, and we can see that over the bullmarket of 2016-2017,
diversity is low.
